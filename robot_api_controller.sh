...
def angle_to_pulse(angle: int) -> int:
    """Converte ângulo (-270…+270) em largura de pulso (50…250)."""
    return int(MIN_PULSE + PULSE_RANGE * (angle + ANGLE_OFFSET) / 180)

def pulse_to_angle(pulse: int) -> int:
    """Converte pulso de volta para ângulo aproximado (apenas para status)."""
    return int(round((pulse - MIN_PULSE) * 180 / PULSE_RANGE) - ANGLE_OFFSET)
...
def move_servo_direct(pin: int, angle: int):
    if SERVO_MAP[pin] != "pos":
        raise HTTPException(status_code=400,
                            detail="Comando direto apenas para servos posicionais")
    pulse = angle_to_pulse(angle)
    run_gpio_command(f"gpio pwm {pin} {pulse}")
    current_state[pin] = pulse          # grava pulso

def move_servo_speed(pin: int, speed: int):
    if SERVO_MAP[pin] != "cr":
        raise HTTPException(status_code=400,
                            detail="Controle de velocidade apenas para servos contínuos")
    stop_pulse = STOP_PULSE_CR.get(pin, 150)
    gain       = GAIN_CR.get(pin, 2)
    pulse      = max(0, min(PWM_RANGE, stop_pulse + speed * gain))
    run_gpio_command(f"gpio pwm {pin} {pulse}")
    current_state[pin] = pulse          # grava pulso (não velocidade)
...
async def get_servo_status():
    ...
    for pin, servo_type in SERVO_MAP.items():
        ...
        value = pulse_to_angle(pulse) if servo_type == "pos" else pulse
    ...
    return {"system_initialized": is_initialized, "servos": servos}
