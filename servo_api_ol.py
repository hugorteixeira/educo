#!/usr/bin/env python3
"""
API REST para Controle de Servos - Orange Pi Zero 2W
Versão corrigida que replica fielmente o comportamento do script bash
"""

from fastapi import FastAPI, HTTPException, BackgroundTasks
from pydantic import BaseModel, Field, validator
from typing import Dict, List, Optional, Union
from math import trunc
import subprocess
import asyncio
import os
import tempfile
import time
from enum import Enum

# ============================================================
# CONFIGURAÇÕES (espelho exato do bash original)
# ============================================================
SERVO_MAP = {
    2: "pos", 9: "pos", 21: "pos", 22: "pos"
}
MIN_PULSE = 50
MAX_PULSE = 250
PULSE_RANGE = MAX_PULSE - MIN_PULSE  # 200
ANGLE_MIN = -270
ANGLE_MAX = 270
ANGLE_OFFSET = 90
STOP_PULSE_CR = {2: 140}
GAIN_CR = {2: 2}
PWM_FREQ = 192
PWM_RANGE = 2000
SMOOTH_STEPS = 40
STEP_DELAY = 0.02

# Estado atual dos servos (valores PWM, não ângulos!)
current_pulse: Dict[int, int] = {}
is_initialized = False

app = FastAPI(
    title="Servo Control API",
    description="API REST para controle de servos via Orange Pi Zero 2W",
    version="1.0.1"
)

# ============================================================
# MODELOS PYDANTIC
# ============================================================
class ServoType(str, Enum):
    POSICIONAL = "pos"
    CONTINUO = "cr"

class ServoCommand(BaseModel):
    pin: int = Field(..., description="Número do pino GPIO")
    value: Union[int, float] = Field(..., description="Ângulo (-270 a +270) ou velocidade (-100 a +100)")
    smooth: bool = Field(default=True, description="Movimento suavizado (apenas para servos posicionais)")
    
    @validator('pin')
    def validate_pin(cls, v):
        if v not in SERVO_MAP:
            raise ValueError(f'Pino {v} não configurado. Pinos válidos: {list(SERVO_MAP.keys())}')
        return v
    
    @validator('value')
    def validate_value(cls, v, values):
        if 'pin' in values:
            servo_type = SERVO_MAP[values['pin']]
            if servo_type == "pos" and not (ANGLE_MIN <= v <= ANGLE_MAX):
                raise ValueError(f'Ângulo deve estar entre {ANGLE_MIN} e {ANGLE_MAX}')
            elif servo_type == "cr" and not (-100 <= v <= 100):
                raise ValueError(f'Velocidade deve estar entre -100 e +100')
        return v

class BatchCommand(BaseModel):
    commands: List[str] = Field(..., description="Lista de comandos no formato bash original")

class DemoFile(BaseModel):
    content: str = Field(..., description="Conteúdo do arquivo demo")
    
class ServoStatus(BaseModel):
    pin: int
    type: str
    current_value: Optional[Union[int, float]]
    current_pulse: Optional[int]
    is_initialized: bool

# ============================================================
# FUNÇÕES DE CONTROLE (tradução fiel do bash)
# ============================================================
def run_gpio_command(command: str) -> subprocess.CompletedProcess:
    """Executa comando gpio e retorna resultado"""
    try:
        result = subprocess.run(
            command, 
            shell=True, 
            capture_output=True, 
            text=True, 
            timeout=5
        )
        return result
    except subprocess.TimeoutExpired:
        raise HTTPException(status_code=500, detail="Timeout no comando GPIO")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Erro executando comando: {str(e)}")

def pwm_write(pin: int, value: int):
    """Replica a função pwm_write do bash"""
    # Clamp do valor (igual ao bash)
    if value < 0:
        value = 0
B    if value > PWM_RANGE:
        value = PWM_RANGE
    
    result = run_gpio_command(f"gpio pwm {pin} {value}")
    if result.returncode != 0:
        raise HTTPException(status_code=500, detail=f"Erro no comando PWM: {result.stderr}")
    
    current_pulse[pin] = value

def config_pwm_pin(pin: int):
    """Replica a função config_pwm_pin do bash"""
    # Ignora erro no mode (igual ao bash: 2>/dev/null || true)
    run_gpio_command(f"gpio mode {pin} pwm 2>/dev/null || true")
    
    result = run_gpio_command(f"gpio pwm-ms {pin}")
    if result.returncode != 0:
        raise HTTPException(status_code=500, detail=f"Erro configurando PWM-MS: {result.stderr}")
    
    result = run_gpio_command(f"gpio pwmc {pin} {PWM_FREQ}")
    if result.returncode != 0:
        raise HTTPException(status_code=500, detail=f"Erro configurando clock: {result.stderr}")
    
    result = run_gpio_command(f"gpio pwmr {pin} {PWM_RANGE}")
    if result.returncode != 0:
        raise HTTPException(status_code=500, detail=f"Erro configurando range: {result.stderr}")

def init_servo_system():
    """Replica fielmente a função init_servo do bash"""
    global is_initialized
    
    # Verifica se gpio está disponível
    result = run_gpio_command("command -v gpio")
    if result.returncode != 0:
        raise HTTPException(status_code=500, detail="Comando 'gpio' não encontrado")
    
    print("Inicializando…")
    
    # pulso de repouso para cada tipo
    centre = MIN_PULSE + PULSE_RANGE // 2  # 150
    
    for pin, servo_type in SERVO_MAP.items():
        # 1) coloca em PWM e já configura clock e range
        config_pwm_pin(pin)
        
        # 2) valor inicial = repouso (NÃO zero!)
        if servo_type == "pos":
            pwm_write(pin, centre)  # 1,5 ms
        else:
            stop_pulse = STOP_PULSE_CR.get(pin, 150)
            pwm_write(pin, stop_pulse)
    
    # 3) espera 25 ms para que o 1º pulso "bom" seja emitido
    time.sleep(0.025)
    
    print("Servos prontos.")
    is_initialized = True
    return True

REAL_STEP_DELAY = 0.045

def move_servo_smooth(pin: int, angle: int):
    """Replica fielmente a função move_smooth do bash"""
    if SERVO_MAP[pin] != "pos":
        raise HTTPException(status_code=400, detail="Movimento suavizado apenas para servos posicionais")
    
    # Validação de ângulo (igual ao bash)
    if not (ANGLE_MIN <= angle <= ANGLE_MAX):
        raise HTTPException(status_code=400, detail=f"Ângulo deve ser {ANGLE_MIN}…{ANGLE_MAX}")
    
    # Cálculo igual ao bash
    norm = angle + ANGLE_OFFSET
    tgt = MIN_PULSE + trunc(PULSE_RANGE * norm / 180)
    
    # Valor atual (valor PWM, não ângulo!)
    curv = current_pulse.get(pin, MIN_PULSE + PULSE_RANGE // 2)
    
    # Cálculo do step (igual ao bash)
    step = int(tgt - curva) // SMOOTH_STEPS
    if abs(step) < 1:
        step = -1 if step < 0 else 1
    
    # Loop de movimento suavizado
    for i in range(SMOOTH_STEPS):
        curv +- step
        pwm_write(pin, curv)
        time.sleep(REAL_STEP_DELAY)
    
    # Posição final exata
    pwm_write(pin, tgt)

def move_servo_direct(pin: int, angle: int):
    """Replica fielmente a função move_direct do bash"""
    if SERVO_MAP[pin] != "pos":
        raise HTTPException(status_code=400, detail="Comando direto apenas para servos posicionais")
    
    # Validação e cálculo igual ao bash
    if not (ANGLE_MIN <= angle <= ANGLE_MAX):
        raise HTTPException(status_code=400, detail=f"Ângulo deve ser {ANGLE_MIN}…{ANGLE_MAX}")
    
    pulse = MIN_PULSE + PULSE_RANGE * (angle + ANGLE_OFFSET) // 180
    pwm_write(pin, pulse)
    print(f"Direto: {pin} ← {angle}° (pulso {pulse})")

def move_servo_speed(pin: int, vel: int):
    """Replica fielmente a função move_speed do bash"""
    if SERVO_MAP[pin] != "cr":
        raise HTTPException(status_code=400, detail="Controle de velocidade apenas para servos contínuos")
    
    # Validação igual ao bash
    if not (-100 <= vel <= 100):
        raise HTTPException(status_code=400, detail="Velocidade -100…+100")
    
    # Cálculo igual ao bash
    stop_pulse = STOP_PULSE_CR.get(pin, 150)
    gain = GAIN_CR.get(pin, 2)
    pulse = stop_pulse + vel * gain
    
    pwm_write(pin, pulse)

def center_all_servos():
    """Replica a função center_all do bash"""
    for pin, servo_type in SERVO_MAP.items():
        if servo_type == "pos":
            move_servo_smooth(pin, 0)
        else:
            move_servo_speed(pin, 0)

def send_raw(pin: int, value: int):
    """Replica a função send_raw do bash"""
    pwm_write(pin, value)
    print(f"RAW {pin} ← {value}")

# ============================================================
# FUNÇÕES PARA BATCH/DEMO (replica run_cmd do bash)
# ============================================================
def run_cmd(cmd_line: str):
    """Replica a função run_cmd do bash"""
    parts = cmd_line.strip().split()
    if not parts or parts[0].startswith('#'):
        return  # vazio ou comentário
    
    cmd = parts[0]
    
    if cmd == "sleep":
        delay = float(parts[1]) if len(parts) > 1 else 0
        time.sleep(delay)
        
    elif cmd == "center":
        center_all_servos()
        
    elif cmd == "raw":
        if len(parts) >= 3:
            send_raw(int(parts[1]), int(parts[2]))
            
    elif cmd == "speed":
        if len(parts) >= 3:
            move_servo_speed(int(parts[1]), int(parts[2]))
            
    elif cmd == "direct":
        if len(parts) >= 3:
            move_servo_direct(int(parts[1]), int(parts[2]))
            
    else:
        # Comando por pino (p=$cmd; val=$1)
        try:
            pin = int(cmd)
            if pin not in SERVO_MAP:
                print("Pino inválido")
                return
            
            if len(parts) < 2:
                return
                
            val = int(parts[1])
            
            if SERVO_MAP[pin] == "pos":
                move_servo_smooth(pin, val)
            else:
                move_servo_speed(pin, val)
                
        except ValueError:
            print(f"Comando inválido: {cmd_line}")

# ============================================================
# ENDPOINTS DA API
# ============================================================
@app.get("/")
async def root():
    return {
        "message": "Servo Control API - Versão Corrigida",
        "version": "1.0.1",
        "status": "Comportamento idêntico ao script bash original",
        "endpoints": ["/docs", "/servo/init", "/servo/move", "/servo/status"]
    }

@app.post("/servo/init")
async def initialize_servos():
    """Inicializa sistema de servos"""
    try:
        init_servo_system()
        return {
            "status": "success", 
            "message": "Servos inicializados",
            "configured_pins": list(SERVO_MAP.keys()),
            "pulse_range": f"{MIN_PULSE}-{MAX_PULSE}",
            "centre_pulse": MIN_PULSE + PULSE_RANGE // 2
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/servo/move")
async def move_servo(command: ServoCommand):
    """Move um servo específico"""
    if not is_initialized:
        raise HTTPException(status_code=400, detail="Sistema não inicializado. Use /servo/init primeiro")
    
    try:
        servo_type = SERVO_MAP[command.pin]
        
        if servo_type == "pos":
            if command.smooth:
                move_servo_smooth(command.pin, int(command.value))
            else:
                move_servo_direct(command.pin, int(command.value))
            
            # Converte pulso atual de volta para ângulo para resposta
            current_pwm = current_pulse[command.pin]
            angle = (current_pwm - MIN_PULSE) * 180 // PULSE_RANGE - ANGLE_OFFSET
            
            return {
                "status": "success",
                "pin": command.pin,
                "angle": command.value,
                "actual_angle": angle,
                "pulse_value": current_pwm,
                "mode": "smooth" if command.smooth else "direct"
            }
        else:  # servo contínuo
            move_servo_speed(command.pin, int(command.value))
            return {
                "status": "success",
                "pin": command.pin,
                "speed": command.value,
                "pulse_value": current_pulse[command.pin]
            }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/servo/center")
async def center_servos():
    """Centraliza todos os servos"""
    if not is_initialized:
        raise HTTPException(status_code=400, detail="Sistema não inicializado")
    
    try:
        center_all_servos()
        return {
            "status": "success", 
            "message": "Todos os servos centralizados",
            "pulse_values": dict(current_pulse)
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/servo/status")
async def get_servo_status():
    """Retorna status atual dos servos"""
    servos = []
    for pin, servo_type in SERVO_MAP.items():
        current_pwm = current_pulse.get(pin)
        current_value = None
        
        if current_pwm is not None and servo_type == "pos":
            # Converte pulso para ângulo
            current_value = (current_pwm - MIN_PULSE) * 180 // PULSE_RANGE - ANGLE_OFFSET
        elif current_pwm is not None and servo_type == "cr":
            # Para servo contínuo, calcula velocidade aproximada
            stop_pulse = STOP_PULSE_CR.get(pin, 150)
            gain = GAIN_CR.get(pin, 2)
            current_value = (current_pwm - stop_pulse) // gain
        
        servos.append(ServoStatus(
            pin=pin,
            type=servo_type,
            current_value=current_value,
            current_pulse=current_pwm,
            is_initialized=is_initialized
        ))
    
    return {
        "system_initialized": is_initialized,
        "pulse_range": f"{MIN_PULSE}-{MAX_PULSE}",
        "pwm_range": PWM_RANGE,
        "servos": servos
    }

@app.post("/servo/raw")
async def send_raw_command(pin: int, value: int):
    """Envia comando PWM direto (equivalente ao 'raw' do bash)"""
    if not is_initialized:
        raise HTTPException(status_code=400, detail="Sistema não inicializado")
    
    if pin not in SERVO_MAP:
        raise HTTPException(status_code=400, detail=f"Pino {pin} não configurado")
    
    try:
        send_raw(pin, value)
        return {
            "status": "success", 
            "pin": pin, 
            "raw_value": value,
            "clamped_value": current_pulse[pin]
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/servo/batch")
async def run_batch_commands(batch: BatchCommand):
    """Executa sequência de comandos (equivalente ao modo batch)"""
    if not is_initialized:
        raise HTTPException(status_code=400, detail="Sistema não inicializado")
    
    results = []
    for cmd_line in batch.commands:
        try:
            if cmd_line.strip():  # ignora linhas vazias
                run_cmd(cmd_line)
                results.append(f"OK: {cmd_line.strip()}")
        except Exception as e:
            results.append(f"ERRO em '{cmd_line.strip()}': {str(e)}")
    
    return {
        "status": "success", 
        "executed_commands": len(batch.commands),
        "results": results,
        "final_pulse_values": dict(current_pulse)
    }

@app.post("/servo/demo")
async def run_demo():
    """Executa demo padrão (tradução da função demo)"""
    if not is_initialized:
        raise HTTPException(status_code=400, detail="Sistema não inicializado")
    
    # Sequência exata do bash
    demo_sequence = [
        "2 45", "2 0", "2 -80", "2 0",
        "9 45", "2 45", "9 0",
        "21 45", "22 0", "9 45",
        "21 -45", "21 45", "22 90", "22 0",
        "center"
    ]
    
    print("=== DEMO PADRÃO ===")
    for cmd_line in demo_sequence:
        run_cmd(cmd_line)
    print("=== FIM DEMO PADRÃO ===")
    
    return {
        "status": "success",
        "message": "Demo padrão executado",
        "commands_executed": demo_sequence,
        "final_pulse_values": dict(current_pulse)
    }

@app.post("/servo/demo/custom")
async def run_custom_demo(demo: DemoFile):
    """Executa demo customizado (replica run_demo_file do bash)"""
    if not is_initialized:
        raise HTTPException(status_code=400, detail="Sistema não inicializado")
    
    lines = demo.content.strip().split('\n')
    limits = {}  # dicionário local de limites
    header_done = False
    executed_commands = []
    
    print("===CUSTOM DEMO===")
    
    for line in lines:
        line = line.strip()
        
        # Ignora linhas vazias/comentários no início
        if not line or line.startswith('#'):
            continue
        
        if not header_done:
            # Verifica formato pin:min:max
            if ':' in line and len(line.split(':')) == 3:
                try:
                    parts = line.split(':')
                    if len(parts) == 3 and all(p.lstrip('-').isdigit() for p in parts):
                        pin, min_val, max_val = map(int, parts)
                        limits[pin] = (min_val, max_val)
                        continue
                except ValueError:
                    pass
            # Primeira linha que não é pin:min:max encerra cabeçalho
            header_done = True
        
        # Ignora linhas vazias/comentários no corpo
        if not line or line.startswith('#'):
            continue
        
        # Processa comando com limites
        parts = line.split()
        if parts and parts[0].isdigit():
            pin = int(parts[0])
            if pin in limits and len(parts) > 1:
                try:
                    val = int(parts[1])
                    min_val, max_val = limits[pin]
                    if val < min_val:
                        val = min_val
                    if val > max_val:
                        val = max_val
                    line = f"{pin} {val}"
                except ValueError:
                    pass
        
        # Executa comando
        try:
            run_cmd(line)
            executed_commands.append(line)
        except Exception as e:
            print(f"Erro em '{line}': {e}")
    
    return {
        "status": "success",
        "message": "Demo customizado executado",
        "limits_applied": limits,
        "commands_executed": executed_commands,
        "final_pulse_values": dict(current_pulse)
    }

# ============================================================
# EXECUÇÃO
# ============================================================
if __name__ == "__main__":
    import uvicorn
    print("Iniciando Servo Control API (Versão Corrigida)...")
    print("Documentação disponível em: http://localhost:8000/docs")
    print("Comportamento idêntico ao script bash original")
    uvicorn.run(app, host="0.0.0.0", port=8000)
