...
# -----------------------------------------------------------------------------
# English aliases for the legacy helpers
# -----------------------------------------------------------------------------
estimate_tokens <- estimar_tokens
load_api_functions <- function(url, pattern = 'openai', allowed_prefixes) {
  carregar_funcoes_api(url, padrao = pattern, prefixos_permitidos = allowed_prefixes)
}
send_openai_message <- enviar_msg_openai
send_message <- function(...) { enviar_msg(...) }
save_response <- salvar_resposta
run_tool_calls <- function(response, openapi_spec_url, base_url = NULL, allowed_prefixes = NULL) {
  executar_resposta_tools(response, openapi_spec_url, base_url = base_url, prefixos_permitidos = allowed_prefixes)
}
get_robot_frame <- function(url, output_dir, allowed_prefixes) {
  obter_frame_robo(url, diretorio = output_dir, prefixos_permitidos = allowed_prefixes)
}
get_robot_status <- function(url) { obter_status_robo(url) }
save_if_different <- salvar_se_diferente
monitor_tools <- monitorar_tools
show_history <- ver_historico
save_history <- salvar_historico
load_history <- carregar_historico
validate_url <- function(url, allowed_prefixes) { validar_url(url, prefixos_permitidos = allowed_prefixes) }
