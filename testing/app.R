...
# =========================
BASE_URL <- "http://192.168.15.7:8000"   # ajuste aqui se necessário
AUTO_MS  <- 2000                         # status atualiza a cada 2s
KEY_REPEAT_MS <- 120                     # repetição ao segurar teclas (ms)

# Passo incremental por part (fração do range). Ajuste individualmente:
# Ex.: 0.05 = 5% do range, 0.25 = 25% do range
DEFAULT_STEP_PCT <- 0.15
PART_STEP_PCT <- list(
  claw   = 0.15,  # ←/→
  height = 0.10,  # ↑/↓
  base   = 0.15,  # A/D (A = esquerda/diminui, D = direita/aumenta)
  reach  = 0.20   # W/S
)
...
safe_pretty <- function(x) {
  tryCatch(jsonlite::toJSON(x, auto_unbox = TRUE, pretty = TRUE),
           error = function(e) paste0("<erro ao formatar JSON: ", e$message, ">"))
}
...
safe_post <- function(base_url, path, body = NULL, timeout = 4) {
  ...
  if (out$status >= 200 && out$status < 300) {
    ...
    out$error <- if (!is.null(err$detail)) {
      paste("Erro:", paste(vapply(err$detail, function(d) d$msg %||% "", character(1L)), collapse = "; "))
    } else paste("Erro HTTP", out$status)
  }
  out
}
...
safe_get_json <- function(base_url, path, query = list(), timeout = 6) {
  ...
  if (out$status >= 200 && out$status < 300) {
    ...
    if (!out$ok) out$error <- "Resposta não é JSON."
  } else {
    err <- tryCatch(resp_body_json(resp, simplifyVector = TRUE), error = function(e) NULL)
    out$error <- if (!is.null(err$detail)) {
      paste("Erro:", paste(vapply(err$detail, function(d) d$msg %||% "", character(1L)), collapse = "; "))
...
# Parsing de range do status: aceita 'min:max' ou vetor de 2 nums
...
norm_part <- function(p) {
  ...
  if (p == "heigth") p <- "height"  # tolera grafia incorreta
  p
}

# Extrai mapa part->(pin, range, value) robusto p/ data.frame ou lista
...
# =========================
# Módulo: Câmera (stream MJPEG)
# =========================
mod_camera_stream_ui <- function(id) {
  ns <- NS(id)
  div(class = "neo-card",
      div(class = "header-bar",
          h4("Câmera (Stream)", class = "hitech-title"),
          span(class = "badge-ghost", "Visão ao vivo")
      ),
      div(class = "camera-frame",
          tags$img(src = paste0(BASE_URL, "/camera/stream"))
      ),
      div(class = "small-note", "Stream MJPEG direto da câmera.")
  )
}

# =========================
# Módulo: Status (auto 2s, alimentado pelo server principal)
# =========================
mod_status_ui <- function(id) {
  ns <- NS(id)
  div(class = "neo-card",
      div(class = "header-bar",
          h4("Status do Robô", class = "hitech-title"),
          span(class = "badge-ghost", "Sistema")
      ),
...
mod_status_server <- function(id, status_reactive) {
  moduleServer(id, function(input, output, session) {
    output$status_json <- renderText({
      dat <- status_reactive()
      if (is.null(dat)) return("// carregando...")
      safe_pretty(dat)
    })
  })
}
...
# =========================
# Módulo: Controles por Teclado/Toque (incrementais)
# - Mapeia parts:
#   claw    -> setas Esquerda/Direita (decrementa/incrementa)
#   height  -> setas Cima/Baixo       (incrementa/decrementa)
#   base    -> teclas A/D             (A = esquerda/decrementa, D = direita/incrementa)
#   reach   -> teclas W/S             (incrementa/decrementa)
# =========================
mod_controls_ui <- function(id) {
  ns <- NS(id)
  div(class = "neo-card",
      div(class = "header-bar",
          h4("Controles (Teclado e Toque)", class = "hitech-title"),
          span(class = "badge-ghost", "Controle")
      ),
      div(id = ns("map_info")),
      hr(class = "hr-soft"),
      div(class = "ctrl-row",
          div(
            ...
            div(class = "small-note", style = "text-align:center; margin-top:8px;",
                "Setas: height (↑/↓) e claw (←/→), incrementais")
          ),
          div(
            div(class = "keypad",
                div(class = "spacer"),
            ...
            div(class = "small-note", style = "text-align:center; margin-top:8px;",
                "W/S: reach (incremental) • A/D: base (A=esq, D=dir, incremental)")
          )
      ),
      div(class = "ctrl-actions",
          actionButton(ns("center"), "Centralizar", class = "hitech-btn"),
...
mod_controls_server <- function(id, status_reactive) {
  moduleServer(id, function(input, output, session) {
    ...
    # Atualiza mapa a cada refresh de status
    observe({
      st <- status_reactive()
      ...
      # Atualiza ângulos locais a partir do status
      if (length(pm)) {
        for (nm in names(pm)) {
      ...
      output$map_info <- renderUI({
        ...
        tagList(
          ...
          div(class = "small-note", "Segure as teclas para controlar. No celular, use os botões acima.")
        )
      })
    })
    
    # Utilitário para enviar ângulo clampado
    ...
    # Função genérica de incremento por part (dir = -1 diminui, +1 aumenta)
    ...
    # Ações de clique (Centralizar/Demo)
...
ui <- page_fluid(
  ...
  fluidRow(
    column(12,
           div(class = "neo-card",
               div(class = "header-bar",
                   div(class = "hitech-title app-title", "Painel de Controle do Robô"),
                   span(class = "badge-soft", "Online")
               ),
               div(class = "small-note",
                   "Stream ao vivo e status a cada 2s. Controle por teclado (e botões para celular) com passos incrementais por part."
               )
           )
    )
  ),
  ...
  div(class = "footer-note", "Feito com Shiny • Estilo neon tech")
)
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    if (res$ok) {
      ...
      status_rv(list(erro = res$error %||% paste("HTTP", res$status)))
    }
